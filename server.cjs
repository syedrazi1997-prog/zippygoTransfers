...
const COUNTRY_NAMES = {
  AF:"Afghanistan", AL:"Albania", DZ:"Algeria", AR:"Argentina", AM:"Armenia", AU:"Australia", AT:"Austria", AZ:"Azerbaijan", BH:"Bahrain", BD:"Bangladesh", BY:"Belarus", BE:"Belgium", BZ:"Belize", BT:"Bhutan", BO:"Bolivia", BA:"Bosnia and Herzegovina", BR:"Brazil", BN:"Brunei", BG:"Bulgaria", KH:"Cambodia", CA:"Canada", CL:"Chile", CN:"China", CO:"Colombia", CR:"Costa Rica", HR:"Croatia", CY:"Cyprus", CZ:"Czechia", DK:"Denmark", DO:"Dominican Republic", EC:"Ecuador", EG:"Egypt", EE:"Estonia", ET:"Ethiopia", FI:"Finland", FR:"France", GE:"Georgia", DE:"Germany", GR:"Greece", HK:"Hong Kong", HU:"Hungary", IS:"Iceland", IN:"India", ID:"Indonesia", IR:"Iran", IQ:"Iraq", IE:"Ireland", IL:"Israel", IT:"Italy", JP:"Japan", JO:"Jordan", KZ:"Kazakhstan", KE:"Kenya", KR:"South Korea", KW:"Kuwait", KG:"Kyrgyzstan", LA:"Laos", LV:"Latvia", LB:"Lebanon", LT:"Lithuania", LU:"Luxembourg", MY:"Malaysia", MV:"Maldives", MT:"Malta", MU:"Mauritius", MX:"Mexico", MD:"Moldova", MN:"Mongolia", ME:"Montenegro", MA:"Morocco", MM:"Myanmar", NP:"Nepal", NL:"Netherlands", NZ:"New Zealand", NG:"Nigeria", MK:"North Macedonia", NO:"Norway", OM:"Oman", PK:"Pakistan", PA:"Panama", PE:"Peru", PH:"Philippines", PL:"Poland", PT:"Portugal", QA:"Qatar", RO:"Romania", RU:"Russia", SA:"Saudi Arabia", RS:"Serbia", SG:"Singapore", SK:"Slovakia", SI:"Slovenia", ZA:"South Africa", ES:"Spain", LK:"Sri Lanka", SE:"Sweden", CH:"Switzerland", TW:"Taiwan", TJ:"Tajikistan", TH:"Thailand", TN:"Tunisia", TR:"Turkey", TM:"Turkmenistan", UA:"Ukraine", AE:"United Arab Emirates", GB:"United Kingdom", US:"United States", UZ:"Uzbekistan", VN:"Vietnam", ZM:"Zambia", ZW:"Zimbabwe"
};
...
app.get("/api/airports/search", async (req, res) => {
  try {
    ...
    const scored = airports.map((a) => {
      const code = normalizeSearch(a.code), name = normalizeSearch(a.name), city = normalizeSearch(a.city), countryName = normalizeSearch(COUNTRY_NAMES[a.country] || a.country);
      const combined = `${code} ${name} ${city} ${countryName}`;
      let score = 0;
      if (code === q) score = 120;
      else if (code.startsWith(q)) score = 110;
      else if (name === q) score = 105;
      else if (name.startsWith(q)) score = 95;
      else if (city === q) score = 100;
      else if (countryName === q) score = 70;
      else if (name.includes(q)) score = 85;
      else if (city.startsWith(q)) score = 80;
      else if (city.includes(q)) score = 70;
      else if (combined.includes(q)) score = 60;
      const tokenHits = qTokens.filter((token) => combined.includes(token)).length;
      if (qTokens.length > 1 && tokenHits === qTokens.length) score = Math.max(score, 75 + tokenHits * 4);
      if (qTokens.length > 1 && tokenHits < qTokens.length) score = 0;
      return { ...a, country: COUNTRY_NAMES[a.country] || a.country, countryCode: a.country, countryName: COUNTRY_NAMES[a.country] || a.country, score };
    }).filter((a) => a.score > 0).sort((a, b) => b.score - a.score || a.name.localeCompare(b.name)).slice(0, 15);
...
// One-request destination geocoding for manually entered hotels, cities or areas.
// This is deliberately server-side (not client autocomplete) and cached to reduce public geocoder load.
const geocodeCache = new Map();
app.get("/api/geocode", async (req, res) => {
  const q = String(req.query.q || "").trim();
  if (q.length < 2) return res.status(400).json({ error: "Destination is required." });
  const airportCity = String(req.query.airportCity || "").trim();
  const airportCountry = String(req.query.airportCountry || "").trim();
  const search = [q, airportCity, airportCountry].filter(Boolean).join(", ");
  ...
  try {
    const r = await axios.get("https://nominatim.openstreetmap.org/search", {
      ...
      headers: { "User-Agent": "ZippyGoTransfers/1.2 (+https://zippygo.com)" }, timeout: 15000, validateStatus: () => true
    });
    if (r.status < 200 || r.status >= 300 || !Array.isArray(r.data) || !r.data.length) return res.status(404).json({ error: "Destination could not be located." });
    const item = r.data[0];
    const data = { latitude: Number(item.lat), longitude: Number(item.lon), displayName: String(item.display_name || q) };
    if (!Number.isFinite(data.latitude) || !Number.isFinite(data.longitude)) return res.status(404).json({ error: "Destination coordinates are unavailable." });
    geocodeCache.set(key, { at: Date.now(), data });
    return res.json(data);
  } catch (e) { console.warn("Geocoding failed:", e.message); return res.status(502).json({ error: "Destination search is temporarily unavailable." }); }
});

// Search exact destination suggestions (hotels, landmarks, areas and addresses).
...
app.get("/api/destinations/search", async (req, res) => {
  ...
  const search = [q, airportCity, airportCountry].filter(Boolean).join(", ");
  const key = normalizeSearch(search);
  const cached = destinationSearchCache.get(key);
  if (cached && Date.now() - cached.at < 6 * 60 * 60 * 1000) return res.json({ results: cached.results });
  try {
    const r = await axios.get("https://nominatim.openstreetmap.org/search", {
      params: { q: search, format: "jsonv2", addressdetails: 1, limit: 10, dedupe: 1, extratags: 1 },
      headers: { "User-Agent": "ZippyGoTransfers/1.2 (+https://zippygo.com)" }, timeout: 15000, validateStatus: () => true
    });
    if (r.status < 200 || r.status >= 300 || !Array.isArray(r.data)) return res.json({ results: [] });
    const results = r.data.map((item, i) => ({
      id: String(item.osm_type || "place") + "-" + String(item.osm_id || i),
      name: String(item.name || item.display_name?.split(",")[0] || q),
    ...
    destinationSearchCache.set(key, { at: Date.now(), results });
    res.set("Cache-Control", "public, max-age=300");
    return res.json({ results });
  } catch (e) {
    console.warn("Destination suggestion search failed:", e.message);
    return res.json({ results: [] });
  }
});
  ...
  try {
    if (env("GOOGLE_MAPS_API_KEY")) {
      try {
        ...
        const hotels = googleHotels.map((p, i) => ({ id: p.id || `google-hotel-${i}`, name: p.displayName?.text || "Hotel", address: p.formattedAddress || "", latitude: Number.isFinite(p.location?.latitude) ? p.location.latitude : undefined, longitude: Number.isFinite(p.location?.longitude) ? p.location.longitude : undefined, distanceKm: Number.isFinite(p.location?.latitude) ? Math.round(distanceKm(p.location.latitude, p.location.longitude) * 10) / 10 : 99999 }));
        const areas = googleAreas.map((p, i) => ({ id: p.id || `google-area-${i}`, name: p.displayName?.text || "Area", type: "area", latitude: Number.isFinite(p.location?.latitude) ? p.location.latitude : undefined, longitude: Number.isFinite(p.location?.longitude) ? p.location.longitude : undefined, distanceKm: Number.isFinite(p.location?.latitude) ? Math.round(distanceKm(p.location.latitude, p.location.longitude) * 10) / 10 : 99999 }));
    ...
    const query = `[out:json][timeout:20];(nwr(around:30000,${lat},${lon})[tourism=hotel];nwr(around:60000,${lat},${lon})[place~"^(neighbourhood|suburb|village|town|city)$"];);out center tags;`;
    const overpassEndpoints = ["https://overpass-api.de/api/interpreter", "https://overpass.kumi.systems/api/interpreter"];
    let r; let lastOverpassError;
    for (const endpoint of overpassEndpoints) {
      try {
        r = await axios.post(endpoint, query, { headers: { "Content-Type": "text/plain", "User-Agent": "ZippyGoTransfers/1.2 (+https://zippygo.com)" }, timeout: 25000 });
        if (r.status >= 200 && r.status < 300) break;
        lastOverpassError = new Error(`Overpass returned HTTP ${r.status}`);
      } catch (overpassError) { lastOverpassError = overpassError; }
    }
    if (!r || r.status < 200 || r.status >= 300) throw lastOverpassError || new Error("All OpenStreetMap Overpass servers failed.");
    ...
    for (const place of geoPlaces) {
      ...
      if (!seenAreas.has(key)) {
        seenAreas.add(key);
        areas.push({ id: `geonames-${place.geonameId || key}`, name, type: String(place.fcodeName || place.fcode || "place").toLowerCase(), latitude: Number.isFinite(pLat) ? pLat : undefined, longitude: Number.isFinite(pLon) ? pLon : undefined, distanceKm: Math.round(distance * 10) / 10 });
      }
    }
    for (const item of r.data?.elements || []) {
      ...
      if (tags.tourism === "hotel" && !seenHotels.has(name.toLowerCase())) { seenHotels.add(name.toLowerCase()); hotels.push({ id, name, address: [tags["addr:street"], tags["addr:housenumber"], tags["addr:city"] || city, tags["addr:country"] || country].filter(Boolean).join(", "), latitude: Number.isFinite(itemLat) ? itemLat : undefined, longitude: Number.isFinite(itemLon) ? itemLon : undefined, distanceKm: Math.round(distance * 10) / 10 }); }
      if (tags.place && !seenAreas.has(name.toLowerCase())) { seenAreas.add(name.toLowerCase()); areas.push({ id, name, type: tags.place, latitude: Number.isFinite(itemLat) ? itemLat : undefined, longitude: Number.isFinite(itemLon) ? itemLon : undefined, distanceKm: Math.round(distance * 10) / 10 }); }
    }
    hotels.sort((a,b) => a.distanceKm - b.distanceKm || a.name.localeCompare(b.name)); areas.sort((a,b) => a.distanceKm - b.distanceKm || a.name.localeCompare(b.name));
...
app.post("/api/create-razorpay-order", async (req, res) => {
  ...
  try {
    ...
    const body = req.body || {};
    // ZippyGo accepts INR only. The client sends the exact INR amount
    // displayed at checkout (rupees); Razorpay receives paise below.
    const currency = "INR";
    ...
    const bookingData = {
      ...
      extras: JSON.stringify({
        ...(typeof booking.extras === "string" ? (() => { try { return JSON.parse(booking.extras); } catch { return {}; } })() : (booking.extras || {})),
