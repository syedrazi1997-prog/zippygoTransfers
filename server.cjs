  ...
  try {
    ...
    for (const overpassUrl of OVERPASS_URLS) {
      try {
        ...
        lastDestinationError = overpassError;
      }
    }
    if (!elements.length && lastDestinationError) throw lastDestinationError;
    const hotels = [];
    const areas = [];
...
// Serve the Vite production build when this backend is deployed as a single Node service.
