...
export interface DestinationSearchOption { id: string; name: string; displayName: string; latitude: number; longitude: number; type: string; }

export async function searchDestinationsRemote(query: string, airport?: RemoteAirportOption): Promise<DestinationSearchOption[]> {
  const backend = getBackendUrl();
  if (!backend || query.trim().length < 2) return [];
  const params = new URLSearchParams({ q: query.trim() });
  if (airport?.city) params.set("airportCity", airport.city);
  if (airport?.country) params.set("airportCountry", airport.country);
  const response = await fetch(`${backend}/api/destinations/search?${params.toString()}`);
