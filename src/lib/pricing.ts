...
function routeDistanceMultiplier(distanceKm?: number): number {
  if (!Number.isFinite(distanceKm)) return 1;
  const km = Math.max(0, Number(distanceKm));
  if (km <= 5) return 0.75;
  if (km <= 15) return 0.85;
  if (km <= 30) return 1;
  if (km <= 50) return 1.25;
  if (km <= 80) return 1.6;
  if (km <= 120) return 2;
  if (km <= 180) return 2.7;
  if (km <= 250) return 3.5;
  return Math.min(6, 3.5 + (km - 250) / 150);
}
...
export function calculateVehiclePriceUSD(
  ...
  if (searchParams.serviceType === "transfer") {
    supplierPerPersonUSD = vehicle.basePriceUSD * vehicle.transferMultiplier * routeDistanceMultiplier(searchParams.routeDistanceKm);
  ...
  const unit = isTransfer
    ? searchParams.roundTrip
      ? `per person · return trip · ${passengerCount} passenger${passengerCount === 1 ? "" : "s"}${Number.isFinite(searchParams.routeDistanceKm) ? ` · ${Math.round(searchParams.routeDistanceKm!)} km route` : ""}`
      : `per person · one way · ${passengerCount} passenger${passengerCount === 1 ? "" : "s"}${Number.isFinite(searchParams.routeDistanceKm) ? ` · ${Math.round(searchParams.routeDistanceKm!)} km route` : ""}`
