  ...
  const validateForm = () => {
    ...
    if (searchParams.serviceType === "transfer") {
      if (!flightNumber.trim()) {
        setError("Please enter your arrival flight number.");
        return false;
      }
      if (searchParams.roundTrip && !returnFlightNumber.trim()) {
        setError("Please enter your return flight number.");
        return false;
      }
    }
    if (searchParams.serviceType === "car_hire") {
      const age = Number(driverAge);
      if (!driverAge || !Number.isFinite(age) || age < 18) {
        setError("Please enter a valid driver age (18 or above).");
        return false;
      }
      if (!licenseNumber.trim() || !licenseCountry.trim()) {
        setError("Please enter the driving licence number and issuing country.");
        return false;
      }
    }
    if (searchParams.serviceType === "parking") {
      if (!vehicleRegistration.trim() || !vehicleMakeModel.trim()) {
        setError("Please enter your vehicle registration and make/model.");
        return false;
      }
    }
    return true;
      ...
      const response = await fetch(`${BACKEND_URL}/api/create-razorpay-order`, {
        ...
        body: JSON.stringify({
          ...
          booking: {
            ...
            return_flight_number: searchParams.roundTrip ? returnFlightNumber.trim() : null,
            service_details: {
              ...(searchParams.serviceType === "car_hire" ? {
                driver_age: Number(driverAge),
                licence_number: licenseNumber.trim(),
                licence_country: licenseCountry.trim(),
              } : {}),
              ...
              ...(searchParams.serviceType === "transfer" ? {
                arrival_flight_number: flightNumber.trim(),
                return_flight_number: searchParams.roundTrip ? returnFlightNumber.trim() : null,
                return_airport_code: searchParams.roundTrip ? (searchParams.pickupAirportCode || searchParams.arrivalAirportCode || null) : null,
              } : {}),
            },
            extras,
          },
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
          <div className="lg:col-span-3 space-y-6">
                  ...
                  <>
                    ...
                    {searchParams.roundTrip && (
                      <>
                        ...
                        <InfoRow label="Return From">
                          {searchParams.pickupAirportCode || searchParams.arrivalAirportCode || "Same airport"}
                        </InfoRow>
            ...
            <div className="bg-white rounded-2xl border border-slate-200 p-6">
              ...
              <p className="text-xs text-slate-500 mb-4">These details are required to confirm your service and are shared with the service provider.</p>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <InputField label="Full Name" value={name} onChange={setName} placeholder="John Doe" required />
                <InputField label="Email" value={email} onChange={setEmail} placeholder="john@example.com" type="email" required />
                <InputField label="Phone" value={phone} onChange={setPhone} placeholder="+919177902449" type="tel" required />

                {searchParams.serviceType === "transfer" && (
                  <>
                    <InputField label="Arrival Flight Number" value={flightNumber} onChange={setFlightNumber} placeholder="BA123" required />
                    {searchParams.roundTrip && (
                      <>
                        <InputField label="Return Flight Number" value={returnFlightNumber} onChange={setReturnFlightNumber} placeholder="BA456" required />
                        <div className="sm:col-span-2 rounded-xl bg-sky-50 border border-sky-100 p-3 text-sm text-sky-800">
                          <strong>Return transfer:</strong> Your return pickup will be from the same airport ({searchParams.pickupAirportCode || searchParams.arrivalAirportCode || "airport selected above"}). Please enter the flight number for the return journey.
                        </div>
                      </>
                    )}
                  </>
                ...
                {searchParams.serviceType === "car_hire" && (
                  <>
                    <InputField label="Driver Age" value={driverAge} onChange={setDriverAge} placeholder="30" type="number" required />
                    <InputField label="Driving Licence Number" value={licenseNumber} onChange={setLicenseNumber} placeholder="Licence number" required />
                    <InputField label="Licence Issuing Country" value={licenseCountry} onChange={setLicenseCountry} placeholder="India" required />
                  </>
                )}

                {searchParams.serviceType === "parking" && (
                  <>
                    <InputField label="Vehicle Registration Number" value={vehicleRegistration} onChange={setVehicleRegistration} placeholder="TS09AB1234" required />
                    <InputField label="Vehicle Make / Model" value={vehicleMakeModel} onChange={setVehicleMakeModel} placeholder="Hyundai Creta" required />
                  </>
                )}
              </div>
            </div>
