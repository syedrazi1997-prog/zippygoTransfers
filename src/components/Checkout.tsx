...
export function Checkout({ vehicle, searchParams, currency, onBack, onComplete }: CheckoutProps) {
  ...
  const extrasTotal = Object.entries(extras).reduce(
    (sum, [key, val]) => (val ? sum + EXTRA_PRICES[key as keyof BookingExtras] * MARGIN : sum),
    0
  );
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      const functionUrl = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/razorpay-payment`;
          ...
          <div className="lg:col-span-3 space-y-6">
            <div className="bg-white rounded-2xl border border-slate-200 p-6">
              <h2 className="text-lg font-bold text-slate-900 mb-4 flex items-center gap-2">
                <ServiceIcon className="w-5 h-5 text-sky-500" />
                Booking Details
              </h2>
          ...
          <div className="lg:col-span-2">
            <div className="bg-white rounded-2xl border border-slate-200 p-6 lg:sticky lg:top-24">
              ...
              <div className="mt-4 flex items-center justify-center gap-2 text-xs text-slate-400">
                ...
                Secured by Razorpay · Free cancellation up to 24h
              </div>
            </div>
          </div>
        </div>
