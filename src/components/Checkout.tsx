...
export function Checkout({
  ...
  // Pricing is calculated internally in USD. The payment gateway, however,
  // must receive the amount in the currency shown to the customer.
  const totalInCurrency = convertFromUSD(totalUSD, currency);
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      const response = await fetch(`${BACKEND_URL}/api/create-razorpay-order`, {
        ...
        body: JSON.stringify({
          // IMPORTANT: send the converted customer-facing amount to Razorpay.
          // Previously totalUSD was sent while the selected currency could be INR,
          // causing Razorpay to show/charge ₹120.54 instead of the converted total.
