...
import { formatPrice, convertFromUSD } from "../lib/currencies";
...
interface CheckoutProps {
  ...
  currency: string;
  onBack: () => void;
  onComplete: (bookingRef: string, totalAmount: number) => void;
}
...
export function Checkout({
  ...
  const paymentAmount = Math.round(convertFromUSD(totalUSD, currency) * 100) / 100;
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      // Create the booking as pending before payment starts.
      const bookingData = await databases.createDocument(
        ...
        ID.unique(),
        {
          ...
          currency,
          amount_in_currency: String(paymentAmount),
          extras: JSON.stringify(extras),
          booking_ref: customBookingRef,
        }
      );

      const bookingRef = bookingData.booking_ref || customBookingRef;
      const backendUrl = (
        import.meta.env.VITE_BACKEND_URL ||
        window.location.origin
      ).replace(/\/+$/, "");

      // Keep the booking state locally so it can be restored after PayFlow
      // redirects the customer back to ZippyGo.
      localStorage.setItem(
        "zippygo-pending-payment",
        JSON.stringify({
          bookingRef,
        ...
        body: JSON.stringify({
          ...
          title:
            searchParams.serviceType === "transfer"
              ? "ZippyGo Airport Transfer"
              : searchParams.serviceType === "car_hire"
              ? "ZippyGo Car Hire"
              : "ZippyGo Airport Parking",
          description: `${vehicle.name} · Booking ${bookingRef}`,
        }),
      });

      const data = await response.json().catch(() => ({}));

      // The backend normally returns checkout_url. For compatibility with
      // PayFlow deployments that return only link_id, construct the public
      // hosted-checkout URL in the browser as a final fallback.
      const payflowPublicUrl = String(
        import.meta.env.VITE_PAYFLOW_PUBLIC_URL ||
        "https://payflow-com.onrender.com"
      ).replace(/\/+$/, "");
      const checkoutUrl =
        data.checkout_url ||
        (data.link_id ? `${payflowPublicUrl}/checkout/${encodeURIComponent(String(data.link_id))}` : "");

      if (!response.ok || !checkoutUrl) {
        const providerStatus = data.provider_status ? ` (provider HTTP ${data.provider_status})` : "";
        throw new Error(
          `${typeof data.error === "string" ? data.error : data.error?.message || "Unable to start PayFlow checkout."}${providerStatus}`
        );
      }

      // PayFlow hosts the secure payment page. No gateway secret is exposed in the browser.
      window.location.assign(checkoutUrl);
    } catch (err: any) {
      console.error("PayFlow payment process error:", err);
      const message =
        typeof err === "string"
          ? err
