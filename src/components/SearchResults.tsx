...
export function SearchResults({ searchParams, currency, onSelectVehicle, onBack }: SearchResultsProps) {
  ...
  const days = useMemo(() => {
    ...
    return 1;
  }, [searchParams.returnDate, searchParams.pickupDate]);

  const vehiclesWithPricing = useMemo(() => {
    const MARGIN = 1.05;
    const pax = Math.max(searchParams.passengers, 1);
    return VEHICLES.map((v) => {
      let priceUSD = v.basePriceUSD;
      let unit = "";

      if (searchParams.serviceType === "transfer") {
        const vehiclePrice = v.basePriceUSD * v.transferMultiplier * MARGIN;
        if (searchParams.roundTrip) {
          priceUSD = (vehiclePrice * 2) / pax;
          unit = "per person · return trip";
        } else {
          priceUSD = vehiclePrice / pax;
          unit = "per person · one way";
        }
      } else if (searchParams.serviceType === "car_hire") {
        priceUSD = v.basePriceUSD * v.carHireDailyMultiplier * MARGIN * days;
        unit = `for ${days} day${days > 1 ? "s" : ""}`;
      } else if (searchParams.serviceType === "parking") {
        priceUSD = v.parkingDailyUSD * MARGIN * days;
        unit = `for ${days} day${days > 1 ? "s" : ""}`;
      }

      return { ...v, priceUSD, unit };
    });
  }, [searchParams.serviceType, days, searchParams.roundTrip, searchParams.passengers]);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-6">
          ...
          <p className="text-slate-500 text-sm mt-1">
            All prices include taxes, fees, and free cancellation up to 24h before pickup
          </p>
        </div>
...
function VehicleCard({
  ...
  vehicle: Vehicle & { priceUSD: number; unit: string };
  currency: string;
  onSelect: () => void;
}) {
