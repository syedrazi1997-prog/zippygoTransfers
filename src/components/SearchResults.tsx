  ...
  const vehiclesWithPricing = useMemo(() => {
    return VEHICLES.map((v) => {
      ...
      return {
        ...
        perPersonUSD: pricing.perPersonUSD,
        unit: pricing.unit,
      };
    });
  }, [searchParams]);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-6">
          ...
          <p className="text-slate-500 text-sm mt-1">
            Prices are calculated per passenger. Select 1 passenger for a single-passenger price, or add passengers to multiply the per-person rate. A 5% service margin is included.
          </p>
...
function searchPassengersLabel(unit: string): string {
  const match = unit.match(/(\d+) passenger/);
  return match?.[1] || "1";
}
...
function VehicleCard({
  ...
  vehicle: Vehicle & { priceUSD: number; perPersonUSD: number; unit: string };
  currency: string;
  onSelect: () => void;
}) {
          ...
          <div className="flex items-end justify-between mt-auto pt-4">
            <div>
              ...
              <p className="text-lg font-semibold text-slate-700">
                {formatPrice(vehicle.perPersonUSD, currency)}
              </p>
              <p className="text-2xl font-bold text-slate-900">
                {formatPrice(vehicle.priceUSD, currency)}
              </p>
              <p className="text-xs text-slate-400">Total for {searchPassengersLabel(vehicle.unit)} passenger{searchPassengersLabel(vehicle.unit) === "1" ? "" : "s"}</p>
              <p className="text-[11px] text-slate-400 mt-1">5% margin included · {vehicle.unit}</p>
            </div>
            <button
              onClick={onSelect}
