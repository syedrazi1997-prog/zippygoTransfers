...
import { Plane, Car, ParkingCircle, MapPin, Calendar, Clock, Users, Search, Hotel, Repeat2, Bus } from "lucide-react";
import type { ServiceType, SearchParams, TransferMode } from "../lib/types";
...
export function SearchWidget({ onSearch, defaultService = "transfer" }: SearchWidgetProps) {
  ...
  const isTransfer = serviceType === "transfer" || serviceType === "shuttle";
  ...
  function handleSubmit() {
    ...
    onSearch({
      ...
      transferMode: serviceType === "shuttle" ? "shuttle" : transferMode,
  ...
  return (
    <div className="w-full bg-white rounded-2xl shadow-xl p-6 border border-slate-200">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        {isTransfer && (
          <div>
            <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Transfer Type</label>
            <div className="flex bg-slate-50 border border-slate-200 rounded-xl p-1">
              <button type="button" onClick={() => setTransferMode("private")} className={`flex-1 py-2 rounded-lg text-xs font-bold ${transferMode === "private" ? "bg-white text-sky-600 shadow-sm" : "text-slate-500"}`}>Private</button>
              <button type="button" onClick={() => setTransferMode("shuttle")} className={`flex-1 py-2 rounded-lg text-xs font-bold ${transferMode === "shuttle" ? "bg-white text-sky-600 shadow-sm" : "text-slate-500"}`}>Shuttle</button>
            </div>
          </div>
        )}
