...
import { getAirportFromLocation, getAirportByCode, getHotelsForAirport, getAreasForAirport, searchAirportsRemote, getAirportDestinationsRemote, geocodeDestination, type RemoteAirportOption } from "../lib/data";
...
export function SearchWidget({ onSearch, defaultService = "transfer" }: SearchWidgetProps) {
  ...
  useEffect(() => {
    setSelectedAirport(undefined); setDropoffLocation(""); setHotels([]); setAreas([]); setDestinationCoordinates(undefined);
  ...
  async function chooseAirport(airport: RemoteAirportOption) {
    setSelectedAirport(airport); setPickupLocation(`${airport.code} — ${airport.name}`); setAirportOptions([]); setDropoffLocation(""); setDestinationCoordinates(undefined);
