...
export function SearchWidget({ onSearch, defaultService = "transfer" }: SearchWidgetProps) {
  ...
  const arrivalAirport = useMemo(() => {
    ...
    const city = selectedPickupAirport?.city;
    const hotelMatch = hotels.find((hotel) =>
      `${hotel.name}, ${hotel.area}`.toLowerCase() === dropoffLocation.toLowerCase()
    );
    const destinationCity = hotelMatch?.city || city;
    return destinationCity ? getAirportsForCity(destinationCity)[0] : undefined;
  }, [dropoffLocation, hotels, selectedPickupAirport]);
  ...
  const handleSubmit = () => {
    ...
    onSearch({
      ...
      roundTrip,
      passengers: Math.max(passengers, 2),
