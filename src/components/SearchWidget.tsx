...
import { getHotelsForAirport, AIRPORT_DATALIST } from "../lib/data";
...
export function SearchWidget({ onSearch, defaultService = "transfer" }: SearchWidgetProps) {
  ...
  const hotels = useMemo(() => {
    const airportCode = pickupLocation.split(" - ")[0];
    return getHotelsForAirport(airportCode || "");
  }, [pickupLocation]);

  const handleSubmit = () => {
    const location = serviceType === "parking" ? pickupLocation || dropoffLocation : pickupLocation;
    onSearch({
      serviceType,
  ...
  const canSubmit = pickupLocation && pickupDate && (serviceType !== "transfer" || dropoffLocation);
    ...
    <div className="w-full">
      ...
      <div className="bg-white rounded-2xl shadow-2xl p-4 sm:p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {serviceType === "transfer" && (
            <>
              <Field label="From (Airport)" icon={Plane}>
                <input
                  list="airport-list"
          ...
          {serviceType === "parking" && (
            <Field label="Airport" icon={Plane}>
              <input
                list="airport-list"
                value={pickupLocation}
                onChange={(e) => setPickupLocation(e.target.value)}
