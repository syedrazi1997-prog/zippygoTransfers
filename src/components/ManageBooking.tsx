...
export function ManageBooking({ currency, onBack }: ManageBookingProps) {
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await fetch(`${getBackendUrl()}/api/bookings/find`, {
        method: "POST",
      ...
      if (!response.ok || !data.booking) {
        throw new Error(
          String(data.error || "No booking found with this reference and email.")
        );
      }

      setBooking(data.booking as BookingRecord);
    } catch (err) {
      ...
      setError(
        err instanceof TypeError && /fetch/i.test(err.message)
          ...
          ? err.message
          : "Failed to search for booking. Please try again."
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleCancel = async () => {
    ...
    try {
      const response = await fetch(
        `${getBackendUrl()}/api/bookings/${encodeURIComponent(
          booking.booking_ref
        )}/cancel`,
        {
          method: "PATCH",
      ...
      if (!response.ok) {
        throw new Error(
          String(data.error || "Failed to cancel booking.")
        );
      }
