...
import { PayFlowCheckout } from "./components/PayFlowCheckout";
...
import type { Vehicle, SearchParams, ServiceType, BookingExtras } from "./lib/types";
import { getAirportByCode, getLocalCurrencyForCountry } from "./lib/data";
...
function CancellationPolicyView() {
  return (
    <div className="max-w-4xl mx-auto px-4 py-12 text-slate-700">
      ...
      <p className="mb-4">
        All approved refunds are processed automatically through our payment gateway partner (PayFlow) to the original card or bank account used during checkout.
      </p>
    </div>
  );
}
...
function TermsOfServiceView() {
  return (
    <div className="max-w-4xl mx-auto px-4 py-12 text-slate-700">
      ...
      <p className="mb-4">
        All bookings placed through ZippyGo are subject to confirmation and vehicle availability. Payments are securely processed via authorized payment gateway partners including PayFlow.
      </p>
...
function PrivacyPolicyView() {
  return (
    <div className="max-w-4xl mx-auto px-4 py-12 text-slate-700">
      ...
      <p className="mb-4">
        We do not directly store your credit/debit card credentials. All financial transactions are handled via encrypted payment gateway interfaces (PayFlow) conforming to PCI-DSS standards.
      </p>
...
export default function App() {
  ...
  const [showPayFlowCheckout, setShowPayFlowCheckout] = useState(false);
  ...
  // Restore the booking after PayFlow redirects the customer back to ZippyGo.
  useEffect(() => {
    ...
    if (raw) {
      try {
        ...
        if (pending?.vehicle && pending?.searchParams && pending?.bookingRef) {
          ...
          setCurrency(pending.currency || "USD");
        ...
        console.error("Unable to restore PayFlow payment state:", restoreError);
      }
    }
  ...
  const handleSearch = (params: SearchParams) => {
    const airport = params.pickupAirportCode ? getAirportByCode(params.pickupAirportCode) : undefined;
    const destinationCurrency =
      params.currency ||
      getLocalCurrencyForCountry(params.destinationCountry || airport?.country || "");
    ...
    setCurrency(destinationCurrency);
    setView("results");
  };

  const handleSelectVehicle = (vehicle: Vehicle) => {
    setSelectedVehicle(vehicle);
    setCheckoutDetails(undefined);
    setShowPayFlowCheckout(false);
    setView("checkout");
  };
        ...
        {view === "checkout" && selectedVehicle && searchParams && (
          showPayFlowCheckout ? (
            <PayFlowCheckout
              ...
              onStarted={(ref, total) => { setBookingRef(ref); setConfirmationTotal(total); }}
              onBack={() => setShowPayFlowCheckout(false)}
            />
          ) : (
            <Checkout
              vehicle={selectedVehicle}
              ...
              onPayFlowCheckout={(details) => { setCheckoutDetails(details); setShowPayFlowCheckout(true); }}
            />
          )
        )}
